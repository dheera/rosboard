...
class PosePublisher extends Publisher {
  ...
  onCreate() {
    ...
    ["position", "orientation"].forEach((_class) => {
      var tr = $("<tr></tr>")
        .append(
          $("<td></td>").text(_class).css({
            width: "30%",
            "font-weight": "bold",
            overflow: "hidden",
            "text-overflow": "ellipsis",
          }),
          $("<td></td>").append(this.createFloatInput(_class, "x")),
          $("<td></td>").append(this.createFloatInput(_class, "y")),
          $("<td></td>").append(this.createFloatInput(_class, "z")),
        )
        .appendTo(this.dataTable);

      if (_class === "orientation")
        tr.append($("<td></td>").append(createFloatInput(_class, "w")));
    });
  ...
  beforePublishing() {
    ...
    ["position", "orientation"].forEach((_class) => {
      ["x", "y", "z"].forEach((id) => {
        $("#" + _class + "_" + id).prop("disabled", true);
      });
    });
  }
...
PosePublisher.supportedTypes = ["geometry_msgs/msg/Pose"];
